...
local convar = GetConVar("climbswep2_roll_allweps")
...
hook.Add("OnPlayerHitGround", "ClimbRoll", function(ply, inWater, idc, fallSpeed)
    ...
    if (GetWeaponClass(ply) == "climb_swep2" or convar:GetInt() > 0) and not ply:GetNWBool("ClimbFalling") and not inWater and fallSpeed > 300 and ply:Crouching() then
        net.Start("ClimbRoll")
...
hook.Add("Think", "ClimbSlide", function()
    ...
    for k, ply in pairs(players) do
        if IsValid(ply) and ply:Alive() then
            ...
            if (GetWeaponClass(ply) == "climb_swep2" or convar:GetInt() > 0) and ply:OnGround() and ply:Crouching() and math.abs(ply.ClimbLastVel:Length()) > 300 and ply.ClimbLastVel.z > -10 then
                ply:SetNWBool("ClimbSlide", true)
                ply.ClimbSlideSound:Play()
                ply.ClimbSlideSound:ChangeVolume(0.5, 0)
            elseif not ply:OnGround() or not ply:Crouching() then
                ply:SetNWBool("ClimbSlide", false)
...
hook.Add("GetFallDamage", "ClimbRollPD", function(ply, fallSpeed)
    ...
    elseif fallSpeed < 900 and ply:Crouching() and (ply:GetActiveWeapon():GetClass() == "climb_swep2" or convar:GetInt() > 0) then
        return 0
    end
end)