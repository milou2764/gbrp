...
function ENT:Draw()
	...
	if LocalPlayer():GetPos():Distance(self:GetPos()) < EML_DrawDistance then
		...
		cam.Start3D2D(pos + ang:Up()*8, ang, 0.055)
			...
			draw.SimpleTextOutlined("Contents", "methFont", -45, 6, Color(1, 241, 249, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));
		...
		cam.Start3D2D(pos + ang:Up()*8, ang, 0.040)		
			...
			draw.SimpleTextOutlined("Salt ("..self:GetNWInt("salt")..")", "methFont", -138, 82, saltColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));		
			draw.SimpleTextOutlined("Crystallized Iodine ("..self:GetNWInt("ciodine")..")", "methFont", -138, 114, ciodineColor, TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(25, 25, 25, 100));	
		cam.End3D2D();	
