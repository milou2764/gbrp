...
function ENT:Draw()
    ...
    ang:RotateAroundAxis(ang:Forward(), 180)
        local ArmorIcon = Material("rarmor/armor.png")
...
local function OpenrArmor()
    ...
    f.Paint = function(s, w, h)
        ...
        local frac = math.Clamp(p:Armor()/100, 0, 1)
        ...
        draw.RoundedBox( 10, w*.5 - w*.12, h *.655, (w*.24 * frac), h*.03 - 5, Color(39, 169, 230))
        draw.SimpleText( p:Armor().." / "..100, "TRE28", w*.5, h *.668, Color(255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
    ...
    h.Paint = function(s, w, h) 
    	...
    	draw.SimpleText( "Besoin d'armure ?", "TRE40", w*.22 , h *.2 + add, Color(255, 255, 255), 0, 1 )
    ...
    if p:IsSuperAdmin() then
        ...
        btn.Paint = function(s, w, h)
            col = s:IsHovered() and Color(26, 135, 186) or Color(39, 169, 230)
        ...
        btn.DoClick = function(s)
            ...
            frame.Paint = function(s, w, h)
                ...
                draw.SimpleText("Cost Per Armor Point", "TRE20", w/2, 2, Color( 245, 245, 245, 245 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
            end

            local te = vgui.Create("DTextEntry", frame)
            te:Dock(TOP)
            te:DockMargin(0,5,0,0)
            te:SetValue(e:GetNWInt("CostPerArmor"))
            ...
            dbtn.Paint = function(self, w, h)
                ...
                draw.SimpleText("Confirm Price", "TRE20", w/2,h/2,Color(245,245,245,255),TEXT_ALIGN_CENTER,TEXT_ALIGN_CENTER)
                else
            ...
            dbtn.DoClick = function(s)
                net.Start("SaveArmorConfigChanges")
   ...
   --[[ h.Think = function(s)
	...
	mdl:Dock(LEFT)
	mdl:SetWide(sh * .11)
	mdl:SetModel("models/Humans/Group03/male_02.mdl")
    ...
    local col
    for k,v in SortedPairs(ARMOR_VALUES_MULTIPLIERS) do
        ...
        hp.Paint = function(s,w,h)
            ...
            local maxarmor = 100
            local quantity = math.Round(maxarmor * v)
            local IsMax = (quantity == maxarmor) and true or false
            quantity = IsMax and "100" or quantity
            local price = DarkRP.formatMoney((e:GetNWInt("CostPerArmor") * maxarmor) * v)
...
net.Receive("rArmorMenuOpen",function()
	OpenrArmor()
end)